...
// GET /api/tables/qr/:token — validasi QR meja publik tanpa membocorkan token lain.
router.get('/tables/qr/:token', async (req,res)=>{
  try {
    const result=await query(`SELECT id,table_number,capacity,zone,status,locked_until,photo_url FROM restaurant_tables WHERE qr_token=$1 LIMIT 1`,[req.params.token]);
    if(!result.rowCount) return res.status(404).json({success:false,message:'QR meja tidak valid.'});
    res.json({success:true,table:result.rows[0]});
  } catch(err){ console.error('[BOOKINGS] validate table qr error:',err); res.status(500).json({success:false,message:'Gagal memvalidasi QR meja.'}); }
});

// GET /api/tables/qr — QR meja lengkap, hanya staf terautentikasi.
router.get('/tables/qr', authenticate, requireRole('OWNER','ADMIN'), async (req,res)=>{
  try {
    const result=await query(`SELECT id,table_number,capacity,zone,qr_token,photo_url FROM restaurant_tables ORDER BY table_number ASC`);
    res.json({success:true,tables:result.rows});
  } catch(err){ console.error('[BOOKINGS] get table qr error:',err); res.status(500).json({success:false,message:'Gagal memuat QR meja.'}); }
});
...
router.get('/bookings', authenticate, requireRole('OWNER', 'ADMIN'), async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT b.id, b.booking_code, b.status, b.order_status, b.payment_status, b.total_amount,
...
const bookingValidation = [
  ...
  body('payment_method').optional().isIn(['QRIS_STATIC', 'QRIS', 'CASHIER']).withMessage('Metode pembayaran tidak valid.'),
...
router.post('/bookings', bookingValidation, async (req, res) => {
  ...
  const paymentMethod = req.body.payment_method === 'CASHIER' ? 'CASHIER' : 'QRIS';
  ...
  try {
    const bookingResult = await withTransaction(async (client) => {
      ...
      const isTaken = ['RESERVED','CONFIRMED'].includes(table.status) || isStillLocked;
      ...
      const upcomingReservation = await client.query(`
        SELECT reservation_code,reservation_at,duration_minutes
        FROM reservations
        WHERE table_id=$1 AND status IN ('PENDING','CONFIRMED')
          AND reservation_at <= now() + (COALESCE($2::int,120)::text || ' minutes')::interval
          AND reservation_at + (duration_minutes::text || ' minutes')::interval > now()
        ORDER BY reservation_at ASC LIMIT 1
      `,[table_id,process.env.RESERVATION_DURATION_MINUTES||120]);
      if (upcomingReservation.rowCount) {
        const rr=upcomingReservation.rows[0];
        const err = new Error(`Meja ${table.table_number} sudah dicadangkan untuk reservasi ${rr.reservation_code}.`);
        err.statusCode = 409;
        throw err;
      }
      ...
      const publicToken = require('crypto').randomBytes(24).toString('base64url');

      const insertBooking = await client.query(
        `INSERT INTO bookings
           (booking_code, public_token, table_id, customer_name, customer_phone, guest_count,
        ...
        [bookingCode, publicToken, table_id, customer_name.trim(), customer_phone.trim(), guest_count || 1, totalAmount, expiresAt]
      );
      ...
      await client.query(
        `INSERT INTO payments (booking_id, amount, method, status, locked_amount, expires_at, provider)
         VALUES ($1, $2, $3, 'PENDING', $2, $4, 'MANUAL')`,
        [booking.id, totalAmount, paymentMethod, expiresAt]
      );
    ...
    return res.status(201).json({
      success: true,
      message: paymentMethod === 'CASHIER' ? 'Pesanan dibuat. Silakan bayar di kasir dalam 10 menit.' : 'Meja berhasil dikunci. Silakan selesaikan pembayaran QRIS dalam 10 menit.',
      booking: {
      ...
      payment_instructions: {
        ...
        note: paymentMethod === 'CASHIER'
          ? 'Bayar di kasir restoran. Setelah pembayaran diterima, kasir/admin mengonfirmasi pesanan.'
          : 'Nominal pembayaran dikunci oleh server. Buat QRIS dari tombol pembayaran; status sukses hanya berasal dari webhook provider.',
      },
    });
  } catch (err) {
...
router.get('/bookings/:id/status', async (req, res) => {
  try {
    ...
    if (auth.startsWith('Bearer ')) {
      try { jwt.verify(auth.slice(7), JWT_SECRET); staffAccess = true; } catch (_) {}
    }
    if (!staffAccess && !accessToken) return res.status(401).json({ success:false, message:'Token akses booking diperlukan.' });
    const isUUID = /^[0-9a-f-]{36}$/i.test(identifier);

    const result = await query(
      `SELECT b.id, b.booking_code, b.status, b.order_status, b.payment_status, b.expires_at, b.total_amount,
              b.customer_name, b.guest_count,
              p.method AS payment_method, p.status AS payment_row_status, p.provider AS payment_provider, p.provider_reference, p.qr_image_url, p.qr_payload, p.expires_at AS payment_expires_at,
              t.table_number, t.status AS table_status
       FROM bookings b
       JOIN restaurant_tables t ON t.id = b.table_id
       LEFT JOIN payments p ON p.booking_id = b.id
       WHERE ${isUUID ? 'b.id = $1' : 'b.booking_code = $1'} ${staffAccess ? '' : 'AND b.public_token = $2'}` ,
      staffAccess ? [identifier] : [identifier, accessToken]
    );
  ...
  try {
    const result = await withTransaction(async (client) => {
      const bookingRes = await client.query(
        `SELECT b.id, b.booking_code, b.status, b.table_id, b.total_amount, b.reservation_id, r.reservation_at,
                t.table_number
         FROM bookings b
         JOIN restaurant_tables t ON t.id = b.table_id
      ...
      await client.query(
        `UPDATE bookings
         SET status = 'CONFIRMED', payment_status = 'PAID', order_status = CASE WHEN order_status='PENDING' THEN 'CONFIRMED' ELSE order_status END, kitchen_status = CASE WHEN kitchen_status='PENDING' THEN 'CONFIRMED' ELSE kitchen_status END, confirmed_by = $1, confirmed_at = now()
         WHERE id = $2`,
        [req.user.id, id]
      );

      // 3. Meja hanya ditandai CONFIRMED ketika reservasi sudah dekat waktunya.
      const reservationIsDue = !booking.reservation_id || !booking.reservation_at || new Date(booking.reservation_at).getTime() <= Date.now() + 30*60*1000;
      if (reservationIsDue) {
        await client.query(`UPDATE restaurant_tables SET status='CONFIRMED',locked_until=NULL,current_booking_id=$1 WHERE id=$2`,[id,booking.table_id]);
      }
    ...
    try {
      ...
      console.error('[PRINT] gagal mencetak struk dapur:', printErr);
    ...
    return res.json({
      success: true,
      message: `Pembayaran meja ${result.booking.table_number} terkonfirmasi. Struk dapur ${printOutcome.status === 'SUCCESS' ? 'berhasil' : 'gagal'} dicetak.`,
...
router.post('/bookings/:id/reprint', authenticate, requireRole('OWNER', 'ADMIN'), async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: `Cetak ulang ${printOutcome.status === 'SUCCESS' ? 'berhasil' : 'gagal'}.`,
      print_status: printOutcome.status,
    });
  } catch (err) {
