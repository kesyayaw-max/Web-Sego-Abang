...
const EDITABLE_KEYS = new Set(['qris_static_image_url','qris_bank_name','table_lock_duration_minutes','kitchen_printer_ip','kitchen_printer_port','restaurant_name','restaurant_tagline']);
...
router.patch('/settings/:key', authenticate, requireRole('OWNER'), async (req, res) => {
  try {
    ...
    if (!EDITABLE_KEYS.has(key)) return res.status(400).json({ success:false, message:'Setting tidak diizinkan untuk diubah.' });
...
router.put('/settings', authenticate, requireRole('OWNER'), async (req, res) => {
  try {
    ...
    for (const [key, value] of Object.entries(settings)) {
      if (!EDITABLE_KEYS.has(key)) return res.status(400).json({ success:false, message:`Setting ${key} tidak diizinkan.` });
      if (key === 'table_lock_duration_minutes') { const mins=parseInt(value,10); if (!Number.isInteger(mins) || mins<1 || mins>60) return res.status(400).json({success:false,message:'Durasi kunci meja harus antara 1–60 menit.'}); }
      const result = await query(
        `INSERT INTO restaurant_settings (key, value, updated_by, updated_at)
         VALUES ($1, $2, $3, now())
         ON CONFLICT (key) DO UPDATE
