...
// ---------------------------------------------------------------------
// Autentikasi Print Agent lokal (bukan staf manusia, jadi tidak pakai
// JWT/login, cukup API key statis yang di-generate sekali & disimpan
// di .env backend DAN di config agent). Dikirim lewat header
// 'X-Print-Agent-Key'.
// ---------------------------------------------------------------------
const PRINT_AGENT_API_KEY = process.env.PRINT_AGENT_API_KEY;

if (!PRINT_AGENT_API_KEY && process.env.NODE_ENV === 'production') {
  console.warn('[AUTH] PERINGATAN: PRINT_AGENT_API_KEY belum di-set. Endpoint print-queue akan menolak semua request agent.');
}

function authenticateAgent(req, res, next) {
  const key = req.headers['x-print-agent-key'];

  if (!PRINT_AGENT_API_KEY || !key || key !== PRINT_AGENT_API_KEY) {
    return res.status(401).json({
      success: false,
      code: 'INVALID_AGENT_KEY',
      message: 'Print agent tidak terautentikasi. Periksa PRINT_AGENT_API_KEY.',
    });
  }
...
module.exports = { authenticate, requireRole, authenticateAgent, JWT_SECRET: EFFECTIVE_SECRET };
