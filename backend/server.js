...
const tableManagementRoutes = require('./routes/tableManagement');
...
app.use('/api', tableManagementRoutes); // /api/table-management
app.use('/api', paymentRoutes); // /api/payments/webhook

// Serve the static frontend from the same Railway service during testing.
// This keeps the staging setup simple: one URL serves both the UI and /api.
...
server.listen(PORT, () => {
  console.log(`[SERVER] RM. Sego Abang Pendopo Wonomarto backend v2.2.0 berjalan di port ${PORT}`);
